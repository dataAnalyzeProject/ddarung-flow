...
def test_builds_exactly_the_required_inference_input_keys():
    ...
    assert [row["stationId"] for row in snapshot] == ["ST-1", "ST-2"]
...
def test_database_url_is_not_written_to_snapshot_or_output(tmp_path, monkeypatch, capsys):
    ...
    class Cursor:
        description = [SimpleNamespace(name=name) for name in ("station_id", "collected_at", "available_bike_count", "inventory_status")]
        def execute(self, statement): self.statement = statement
        def fetchall(self): return [("ST-1", datetime(2026, 8, 27, 2, 0, tzinfo=timezone.utc), 1, "NORMAL")]
        def close(self): pass
    class Connection:
