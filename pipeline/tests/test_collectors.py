...
def test_collect_bike_inventory_reads_real_response_and_all_pages():
    ...
    assert result["collection_evidence"] == {
        "status": "COMPLETE",
        "reason": None,
        "page_size": 1000,
        "page_count": 2,
        "expected_row_count": 1001,
        "row_count": 1001,
        "terminal_page_row_count": 1,
    }
...
def test_collect_bike_inventory_rejects_non_integer_provider_total(
    invalid_total, row_count
):
    response = {
        "rentBikeStatus": {
            "list_total_count": invalid_total,
            "RESULT": {"CODE": "INFO-000"},
            "row": [
                {"stationId": f"ST-{number}", "parkingBikeTotCnt": "1"}
    ...
    assert result["collection_evidence"]["reason"] == "PAGE_CONTINUITY_BREAK"


def test_collect_bike_inventory_accepts_strict_integer_provider_total_string():
    response = {
        "rentBikeStatus": {
            "list_total_count": "1",
            "RESULT": {"CODE": "INFO-000"},
            "row": [{"stationId": "ST-1", "parkingBikeTotCnt": "0"}],
        }
    }
    ...
    assert result["collection_evidence"]["status"] == "COMPLETE"
    assert result["collection_evidence"]["expected_row_count"] == 1
...
def test_collect_bike_inventory_marks_invalid_follow_up_page_partial(second_page, reason):
    ...
    result = collect_bike_inventory(
        FakeBikeClient([first_page, second_page]), COLLECTED_AT, page_size=2
    )

    assert result["collection_evidence"]["status"] == "PARTIAL"
    assert result["collection_evidence"]["reason"] == reason
...
def test_collect_bike_inventory_follow_up_transport_failure_is_not_published_as_partial_result():
    ...
    with pytest.raises(SeoulBikeTransportError):
        collect_bike_inventory(
            FollowUpFailureClient(),
            COLLECTED_AT,
            page_size=2,
            sleep=lambda _: None,
            random_source=lambda *_: 0,
        )
