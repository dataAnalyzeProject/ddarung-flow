...
| 팀원 | 주 역할 | 대표 기여 |
|---|---|---|
| **김선호** | PM / 관리 · 통합 | WBS·작업 계약과 우선순위 관리, PR·기준선 검토와 통합, CI/CD·Staging·Runtime Evidence 검증, 최종 발표·인수 근거 정리 |
| **황준형** | 데이터 수집 · 품질 · 모델 평가 | 연도별 재고 데이터 범위·품질 검증, H1~H4 미래 재고·필요 수량 기준선 재현, 모델 비교·평가 및 분포 artifact 검증 |
| **선경원** | Backend · API / DB | OAuth 사용자 저장·갱신, 도착시각·H1~H4 horizon 상태 계산, 예측 조회와 지도·경로·주변 대여소 API, Q&A 등 백엔드·DB 기능 |
...
2. 팀원은 **분리된 브랜치와 PR**에서 담당 영역을 구현하고 테스트·화면·검증 근거를 제출했습니다.
3. 담당 영역 간 API·데이터 계약이 만나는 지점은 PR diff와 실제 동작을 함께 검토해 통합했습니다.
4. 통합 결과는 **GitHub Actions CI → Staging CD → Runtime Evidence** 순서의 검증 근거로 확인했습니다.
