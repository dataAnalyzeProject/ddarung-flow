...
public class QnaController {
    ...
    @GetMapping("/admin/qna/questions") public QnaDtos.PageResponse adminList(@AuthenticationPrincipal PrincipalDetails principal) { List<QnaQuestion> questions = qnaService.getAllQuestions(); return new QnaDtos.PageResponse(questions.stream().map(q -> response(q, principal.getUsers().getId())).toList(), 0, questions.size(), questions.size()); }
    @PostMapping("/admin/qna/questions/{id}/answer") public QnaDtos.QuestionResponse answer(@AuthenticationPrincipal PrincipalDetails principal, @PathVariable Long id, @RequestBody QnaDtos.AnswerRequest request) { qnaService.addAnswer(id, principal.getUsers().getId(), principal.getUsers().getDisplayName(), request.body()); return response(qnaService.getQuestion(id), principal.getUsers().getId()); }
    @PostMapping("/admin/qna/questions/{id}/hide") public QnaDtos.QuestionResponse hide(@AuthenticationPrincipal PrincipalDetails principal, @PathVariable Long id) { return response(qnaService.hideQuestion(id), principal.getUsers().getId()); }
