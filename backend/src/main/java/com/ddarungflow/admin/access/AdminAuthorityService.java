...
public class AdminAuthorityService {
    private static final Set<AdminPermission> OPS_PRIMARY_ROUTES = Set.of(
            AdminPermission.OPS_DASHBOARD_READ, AdminPermission.OPS_RISK_MAP_READ,
            AdminPermission.OPS_CANDIDATE_READ, AdminPermission.OPS_REPORT_EXPORT,
            AdminPermission.OPS_SCENARIO_READ);
    private static final Set<AdminPermission> DATA_ROUTES = Set.of(AdminPermission.DATA_STATUS_READ);
    private static final Set<AdminPermission> OPS_SECONDARY_ROUTES = Set.of(AdminPermission.OPS_ANALYSIS_READ);
    ...
    public static AdminConsole defaultConsole(Set<AdminPermission> permissions) {
        if (permissions.stream().anyMatch(OPS_PRIMARY_ROUTES::contains)) return AdminConsole.OPS;
        if (permissions.stream().anyMatch(DATA_ROUTES::contains)) return AdminConsole.DATA;
        if (permissions.stream().anyMatch(OPS_SECONDARY_ROUTES::contains)) return AdminConsole.OPS;
