...
@Service public class ModelPerformanceService {
 ...
 public AdminModelPerformanceDtos.Response find(String artifactSha256) { ModelPerformanceRun run = selectRun(artifactSha256); var p=run.getPayload(); return new AdminModelPerformanceDtos.Response(run.getArtifactSha256(),run.getModelVersion(),run.getGeneratedAt(),p.get("evaluation"),p.get("combinations"),p.get("segments"),p.get("calibrationBins")); }
