...
- `TASK-291`: lifecycle through `COMMIT_PUSH_PR -> EXACT_HEAD_CI`, then stop before merge because its newer task-specific prompt says `merge하지 않는다`; predecessors `TASK-304` and `TASK-305`; allowlist only new `frontend/src/features/consumer-r2/shared/**`, new `frontend/src/features/consumer-r2/styles/**`, and new focused foundation tests; Sol high implementer, Visual Reviewer Sol high/xhigh, ceiling xhigh; automatic visual and asset protocols with desktop/tablet/mobile plus success/non-success captures; an asset gap is mandatory work, never a visual PASS exception. Merge and same-SHA staging remain unresolved future gates.
