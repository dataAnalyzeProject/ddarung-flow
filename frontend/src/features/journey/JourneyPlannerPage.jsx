...
export default function JourneyPlannerPage({ authState, onNavigate }) {
  ...
  return <main className="journey-page"><section className="journey-hero"><p>UI-24 · Journey Planner</p><h1>여정 조건을 입력하세요</h1><span>로그인 후 실제 여정 API로 후보를 만듭니다.</span></section><form className="journey-form" onSubmit={submit}>
    ...
    <div className="journey-grid">{placeInput('origin', '출발 장소')}{placeInput('destination', '최종 목적지 (선택)')}<label>필요한 자전거 수<input aria-label="필요한 자전거 수" type="number" min="1" max="5" value={input.requiredBikeCount} onChange={(e) => update('requiredBikeCount', e.target.value)} required /></label><label>최대 이동 시간(분)<input aria-label="최대 이동 시간" type="number" min="1" value={input.maxJourneyMinutes} onChange={(e) => update('maxJourneyMinutes', e.target.value)} required /></label><label>출발 희망 시각<input aria-label="출발 희망 시각" name="departureAt" type="datetime-local" value={input.departureAt} onInput={updateDepartureAt} onChange={updateDepartureAt} /></label><label>회피 조건<select aria-label="회피 조건" value={input.avoid[0] || ''} onChange={(e) => update('avoid', e.target.value ? [e.target.value] : [])}><option value="">없음</option><option value="RAIN">비</option><option value="UNLIT_ROUTE">어두운 길</option></select></label></div>
