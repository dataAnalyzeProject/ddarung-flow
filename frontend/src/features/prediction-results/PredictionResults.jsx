...
export default function PredictionResults({ result = normalResult, onModify = () => {} }) {
  const [requiredBikeCount, setRequiredBikeCount] = useState(result.requiredBikeCount);
  ...
  if (!result.candidates?.length) return <main className="prediction-results-shell empty-state">
    <div><span>NO PREDICTION RESULT</span><h1>조건에 맞는 예측 결과가 없습니다.</h1><p>출발지, 목적지 또는 예상시간을 수정한 뒤 다시 확인해 주세요.</p><button type="button" onClick={onModify}>입력 수정</button></div>
  </main>;
  ...
  return <main className="prediction-results-shell">
    ...
    <section className="prediction-results-heading"><div><p>PREDICTION COMPLETE</p><h1>목적지 주변 예측 결과</h1><span>후보별 도착시각과 필요한 수량의 대여 가능성을 비교하세요.</span></div><label>필요 수량<select aria-label="필요 수량" value={requiredBikeCount} onChange={(event) => setRequiredBikeCount(Number(event.target.value))}>{[1, 2, 3, 4, 5].map((count) => <option value={count} key={count}>{count}대</option>)}</select></label></section>
    <section className="prediction-candidate-grid" aria-label="예측 후보 목록">
      {result.candidates.map((candidate) => <PredictionCard key={candidate.stationId} candidate={candidate} requiredBikeCount={requiredBikeCount} selected={candidate.stationId === selectedCandidate.stationId} onSelect={setSelectedStationId} onToggleDetails={toggleDetails} detailsOpen={openDetailsId === candidate.stationId} />)}
    </section>
    {openDetailsId && <PredictionDetails candidate={result.candidates.find((candidate) => candidate.stationId === openDetailsId)} />}
    {alternatives.length > 0 && <section className="prediction-alternatives"><header><div><span>LOW AVAILABILITY</span><h2>대체 대여소를 확인하세요.</h2></div><p>선택한 대여소의 가능성이 낮아 다른 후보를 안내합니다.</p></header><div>{alternatives.map((candidate) => <PredictionCard key={candidate.stationId} candidate={candidate} requiredBikeCount={requiredBikeCount} selected={false} onSelect={setSelectedStationId} onToggleDetails={toggleDetails} detailsOpen={openDetailsId === candidate.stationId} alternative />)}</div></section>}
