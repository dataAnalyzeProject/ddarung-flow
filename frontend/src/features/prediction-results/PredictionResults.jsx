...
import InventoryReferenceTime from "./components/InventoryReferenceTime";
import PredictionSortControls from "./components/PredictionSortControls";
...
export default function PredictionResults({ result = normalResult, onEditInput = () => {}, onSelectStation = () => {} }) {
  ...
  const sortedCandidates = useMemo(() => [...(result.candidates ?? [])].sort((a, b) => {
  ...
  }), [result.candidates, sortBy]);

  const inventoryReferenceTime = useMemo(() => {
    const collectedTimes = (result.candidates ?? [])
  ...
  const selectedCandidate = sortedCandidates.find((candidate) => candidate.stationId === selectedStationId) ?? sortedCandidates[0];
  const alternatives = selectedCandidate.availabilityLevel === "LOW" ? sortedCandidates.filter((candidate) => candidate.stationId !== selectedCandidate.stationId).slice(0, 5) : [];
  ...
  return <main className="prediction-results-shell">
    ...
    <section className="prediction-results-heading"><div><p>PREDICTION COMPLETE</p><h1>목적지 주변 예측 결과</h1><span>후보별 도착시각과 {result.requiredBikeCount}대 대여 가능성을 비교하세요.</span></div><PredictionSortControls value={sortBy} onChange={setSortBy} /></section>
    <InventoryReferenceTime referenceTime={inventoryReferenceTime} status={inventoryStatus} />
    <section className="prediction-candidate-grid" aria-label="예측 후보 목록">
      {sortedCandidates.map((candidate) => <PredictionCard key={candidate.stationId} candidate={candidate} requiredBikeCount={result.requiredBikeCount} selected={candidate.stationId === selectedCandidate.stationId} onSelect={selectStation} onToggleDetails={toggleDetails} detailsOpen={openDetailsId === candidate.stationId} />)}
    </section>
