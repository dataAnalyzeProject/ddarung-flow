...
function Users() {
  ...
  return <><div className="admin-page-title"><div><p className="admin-eyebrow">UI-ADMIN-02</p><h1>사용자 · 권한</h1><span>실제 API의 공개 UUID, 표시 이름, 역할만 표시합니다.</span></div></div>
    <form className="admin-filter-row" onSubmit={(event) => { event.preventDefault(); load(query); }}><input aria-label="사용자 검색" placeholder="표시 이름 검색" value={query} onChange={(event) => setQuery(event.target.value)} /><Button kind="primary">검색</Button><span>총 {items.length}명</span></form>
