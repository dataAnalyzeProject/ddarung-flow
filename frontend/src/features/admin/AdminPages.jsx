...
function ModelOps({ actorRole }) {
  const [models, setModels] = useState([]); const [state, setState] = useState("loading"); const [actionError, setActionError] = useState(""); const [busy, setBusy] = useState(null);
  const load = useCallback(() => { setState("loading"); listAdminModels().then((items) => { setModels(items); setState(items.length ? "success" : "empty"); }).catch((error) => setState(error.status === 401 ? "unauthorized" : error.status === 403 ? "forbidden" : "error")); }, []);
  useEffect(() => { load(); }, [load]);
...
export function AdminPage({ menuId, actorRole, fixtureData = fixture, onAction = () => {} }) {
  const pages = { dashboard: <Dashboard data={fixtureData} />, users: <Users />, export: <Export actorRole={actorRole} />, audit: <Audit />, modelops: <ModelOps actorRole={actorRole} />, qna: <Qna data={fixtureData} actorRole={actorRole} onAction={onAction} /> };
