...
- 활성화한 테스트:
  1. FE-4.2 경로 방식 선택값을 승인된 형태로 전달한다 (`PASS`)
  2. FE-4.2 직접 시간 방식 선택값을 승인된 형태로 전달한다 (`PASS`)
  3. FE-4.2 두 글자 미만 검색어는 검색하지 않는다 (`PASS`)
  4. FE-4.2 결과 없음과 오류 상태를 구분한다 (`PASS`)
  5. FE-4.2 필수 선택 전에는 계속하기를 누를 수 없다 (`PASS`)
- 추가한 키보드 테스트:
  - `FE-4.2 키보드만 사용해 탭, 검색 결과, 조건 선택과 계속하기를 진행한다` (`PASS`)
