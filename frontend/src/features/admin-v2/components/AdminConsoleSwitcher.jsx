...
export default function AdminConsoleSwitcher({ consoles, activeConsole, onSelect }) {
  return <nav aria-label="관리자 콘솔 전환"><div className="admin-v2-console-switcher">
    {consoles.map((consoleId) => <button key={consoleId} type="button" aria-current={consoleId === activeConsole ? 'page' : undefined} onClick={() => onSelect(consoleId)}>{LABELS[consoleId]}</button>)}
  </div></nav>;
}
