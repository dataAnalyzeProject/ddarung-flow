...
export default function ModelOverview({ createAdapter }) {
  ...
  return <main className="model-overview-page" aria-label="모델 운영 현황">
    ...
    <section className="model-overview-service-card" aria-labelledby="service-model-heading"><span className="model-overview-service-icon" aria-hidden="true"><ServiceModelIcon /></span><div className="model-overview-service-identity"><p>서비스 모델</p><h2 id="service-model-heading">{runtime?.state === 'SUCCESS' ? runtime.data.modelVersion : 'UNKNOWN'}</h2></div>{runtime?.state === 'SUCCESS' ? <dl className="model-overview-runtime-meta"><div><dt>Serving</dt><dd>LIVE INFERENCE</dd></div><div><dt>Artifact SHA</dt><dd>{shortSha(runtime.data.artifactSha256)}</dd></div><div><dt>Source</dt><dd>{runtime.data.modelSource}</dd></div><div><dt>지원 범위</dt><dd>H{runtime.data.supportedHorizons.join(', H')} · 수량 {runtime.data.supportedQuantities.join(', ')}</dd></div><div><dt>Loaded</dt><dd>{new Date(runtime.data.loadedAt).toLocaleString('ko-KR')}</dd></div></dl> : <SourceState source={runtime} permission="MODEL_METRICS_READ" label="실시간 inference runtime" />}</section>
