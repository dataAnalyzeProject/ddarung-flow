...
test('uses stationNumber for marker selection and cleans up overlays', () => {
  ...
  const maps = {
    ...
    event: { addListener: jest.fn((_, name, handler) => { handlers[name] = handler; return handler; }), removeListener: jest.fn() },
  };
  ...
  adapter.destroy(); expect(overlays[0].setMap).toHaveBeenCalledWith(null);
});
