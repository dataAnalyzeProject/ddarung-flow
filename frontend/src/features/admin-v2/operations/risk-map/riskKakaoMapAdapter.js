...
const MARKER_BY_BAND = {
  CRITICAL: { className: 'risk-map-marker--critical', label: 'C' },
  HIGH: { className: 'risk-map-marker--high', label: 'H' },
  WATCH: { className: 'risk-map-marker--watch', label: 'W' },
  LOW: { className: 'risk-map-marker--low', label: 'L' },
};
function markerPresentation(item) {
  ...
  return { className: 'risk-map-marker--unknown', label: '?', ariaRisk: item.dataState && item.dataState !== 'NORMAL' ? item.dataState : '판단 정보 부족' };
}
