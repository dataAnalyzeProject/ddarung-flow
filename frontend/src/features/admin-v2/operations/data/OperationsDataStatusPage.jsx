...
export default function OperationsDataStatusPage({ createAdapter }) {
  ...
  return <main className="operations-data-page" aria-label="운영 데이터 상태"><header className="operations-data-header"><div><p className="operations-data-eyebrow">UI-OPS-05 · DATA_STATUS_READ</p><h1>운영 데이터 상태</h1><p>현재 운영 위험 판단에 쓰는 데이터가 얼마나 최신이고 얼마나 넓게 채워졌는지 확인합니다.</p></div><dl><Metric label="전체 데이터 상태"><State value={result?.dataState} /></Metric><Metric label="기준 시각">{time(result?.referenceTime)}</Metric><Metric label="생성 시각">{time(result?.generatedAt)}</Metric></dl></header><StatusCriteria /><p className="operations-data-safety-notice">결측 데이터는 정상 값으로 대체하지 않습니다.</p><SourceSummary result={result} /><Inventory inventory={result?.inventory} /><RuntimeAnalysis runtimeAnalysis={result?.runtimeAnalysis} /><RuntimeAnalysisGuidance limitations={result?.limitations} /><div className="operations-data-supporting"><Prediction prediction={result?.prediction} /><Profile profile={result?.profile} /></div><Limitations limitations={result?.limitations} /></main>;
}
