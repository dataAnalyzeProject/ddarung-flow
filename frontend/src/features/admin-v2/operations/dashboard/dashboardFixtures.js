...
function items(dataState = 'NORMAL') {
  return stationNames.map((name, index) => ({
    station: { stationNumber: String(1001 + index), name, coordinates: { latitude: 37.56 + (index * 0.007), longitude: 126.97 + (index * 0.008) }, currentBikes: index + 1, capacity: null },
...
export function dashboardFixture(name = 'SUCCESS') {
  switch (name) {
    ...
    case 'DELAYED': return { overview: overview('DELAYED'), risk: { referenceTime: '2026-08-30T08:40:00+09:00', dataState: 'DELAYED', items: items('DELAYED') }, riskError: null };
    case 'MISSING': return { overview: overview('MISSING'), risk: { referenceTime: '2026-08-30T09:00:00+09:00', dataState: 'MISSING', items: items('MISSING') }, riskError: null };
    case 'RISK_DELAYED': return { overview: overview(), risk: { referenceTime: '2026-08-30T08:40:00+09:00', dataState: 'DELAYED', items: items('DELAYED') }, riskError: null };
    case 'RISK_MISSING': return { overview: overview(), risk: { referenceTime: '2026-08-30T09:00:00+09:00', dataState: 'MISSING', items: items('MISSING') }, riskError: null };
    case 'INSUFFICIENT_DATA': return { overview: overview('INSUFFICIENT_DATA'), risk: { referenceTime: '2026-08-30T09:00:00+09:00', dataState: 'INSUFFICIENT_DATA', items: [] }, riskError: null };
    case 'UNAVAILABLE': return { overview: overview('UNAVAILABLE'), risk: { referenceTime: '2026-08-30T09:00:00+09:00', dataState: 'UNAVAILABLE', items: [] }, riskError: null };
    case 'EMPTY': return { overview: overview('NORMAL', { coverage: { activeStationCount: 0, inventoryAvailableCount: 0, predictionAvailableCount: 0, profileAvailableCount: 0 }, limitations: ['NO_ACTIVE_PUBLIC_STATIONS'] }), risk: { referenceTime: '2026-08-30T09:00:00+09:00', dataState: 'NORMAL', items: [] }, riskError: null };
    ...
    default: return { overview: overview(), risk: { referenceTime: '2026-08-30T09:00:00+09:00', dataState: 'NORMAL', items: items() }, riskError: null };
  }
}
