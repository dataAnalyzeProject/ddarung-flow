...
export default function StationDetailPage({
  ...
  return (
    <ConsumerR2Theme className="cr22-station">
      <ConsumerAppHeader activeItem="nearby" authState={authState} onAccount={() => onNavigate?.("mypage")} onLogin={() => window.location.assign("/login")} onNavigate={onNavigate} userName={user?.name} userTier={user?.tier} />
      <ConsumerContainer as="main" id="main-content" className="cr22-station__content">
        <button className="cr22-station__back" type="button" onClick={() => onNavigate?.("nearby")}><ConsumerIcon name="arrowRight" size={16} /> 검색 결과로 돌아가기</button>
